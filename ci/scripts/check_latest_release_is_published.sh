...
# Copyright 2016-Present flashcat, Inc.
#
...
tag_name=$(curl -s -H "Authorization: Bearer $GITHUB_TOKEN" \
     ...
     https://api.github.com/repos/flashcat/fc-sdk-android/releases/latest | jq -r .tag_name)
...
for artifactId in $(./gradlew -q listAllPublishedArtifactIds); do
  artifact_url="https://repo1.maven.org/maven2/com/flashcathq/$artifactId/$tag_name/$artifactId-$tag_name.aar"
