...
internal class ExposuresRequestFactoryTest {
    ...
    fun `M create a proper request W create()`(
        ...
        // When
        val request = testedFactory.create(fakeFlashcatContext, executionContext, batchData, metaData)

        // Then
        requireNotNull(request)
        assertThat(request.url).isEqualTo("${fakeFlashcatContext.site.intakeEndpoint}/api/v2/exposures")
    ...
    fun `M generate unique request IDs W create() { multiple calls }`(
        ...
        // When
        val request1 = testedFactory.create(fakeFlashcatContext, executionContext, batchData, metadata)
        val request2 = testedFactory.create(fakeFlashcatContext, executionContext, batchData, metadata)
    ...
    fun `M use custom endpoint W create() { custom endpoint provided }`(
        ...
        // When
        val request = factoryWithCustomEndpoint.create(fakeFlashcatContext, executionContext, batchData, metadata)
