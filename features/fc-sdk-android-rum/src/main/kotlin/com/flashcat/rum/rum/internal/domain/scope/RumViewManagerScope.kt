...
import com.flashcat.rum.rum.internal.metric.SessionEndedMetric
...
internal class RumViewManagerScope(
    ...
    override fun handleEvent(
        ...
        if (event is RumRawEvent.ApplicationStarted &&
            ...
            startApplicationLaunchView(event, flashcatContext, writeScope, writer)
            return this
        }

        delegateToChildren(event, flashcatContext, writeScope, writer)

        if (event is RumRawEvent.StartView && !stopped) {
            startForegroundView(event, flashcatContext, writeScope, writer)
            ...
            handleOrphanEvent(event, flashcatContext, writeScope, writer)
        }
    ...
    private fun startApplicationLaunchView(
        ...
        viewScope.handleEvent(event, flashcatContext, writeScope, writer)
    ...
    private fun delegateToChildren(
        ...
        while (iterator.hasNext()) {
            ...
            val result = childScope.handleEvent(event, flashcatContext, writeScope, writer)
            if (result == null) {
                iterator.remove()
            }
        }
    ...
    private fun handleOrphanEvent(
        ...
        if (event is RumRawEvent.AddViewLoadingTime) {
            ...
            handleBackgroundEvent(event, flashcatContext, writeScope, writer)
        } else {
    ...
    private fun startForegroundView(
        ...
        applicationDisplayed = true
        childrenScopes.add(viewScope)
        viewScope.handleEvent(RumRawEvent.KeepAlive(), flashcatContext, writeScope, writer)
