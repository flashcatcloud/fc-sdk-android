...
internal class RumResourceScope(
    ...
    override fun handleEvent(
        ...
        when (event) {
            ...
            is RumRawEvent.AddResourceTiming -> onAddResourceTiming(event, flashcatContext, writeScope, writer)
            is RumRawEvent.StopResource -> onStopResource(event, flashcatContext, writeScope, writer)
            is RumRawEvent.StopResourceWithError -> onStopResourceWithError(event, flashcatContext, writeScope, writer)
    ...
    private fun onStopResource(
        ...
        if (!(waitForTiming && timing == null)) {
            sendResource(kind, event.statusCode, event.size, event.eventTime, flashcatContext, writeScope, writer)
        }
    }
    ...
    private fun onAddResourceTiming(
        ...
        timing = event.timing
        if (stopped && !sent) {
            sendResource(kind, statusCode, size, event.eventTime, flashcatContext, writeScope, writer)
        }
    }
        ...
        sdkCore.newRumEventWriteOperation(flashcatContext, writeScope, writer) {
            ...
            ResourceEvent(
                ...
                device = ResourceEvent.Device(
                    type = flashcatContext.deviceInfo.deviceType.toResourceSchemaType(),
