...
internal class RumApplicationScope(
    ...
    override fun handleEvent(
        ...
        if (activeSession == null && isInteraction) {
            startNewSession(event, flashcatContext, writeScope, writer)
        }

        if (event !is RumRawEvent.SdkInit && !isAppStartedEventSent) {
            sendApplicationStartEvent(event.eventTime, flashcatContext, writeScope, writer)
        }
    ...
    private fun delegateToChildren(
        ...
        while (iterator.hasNext()) {
            val result = iterator.next().handleEvent(event, flashcatContext, writeScope, writer)
            if (result == null) {
                iterator.remove()
            }
        }
    ...
    private fun startNewSession(
        ...
        if (event !is RumRawEvent.StartView) {
            lastActiveViewInfo?.let {
                ...
                newSession.handleEvent(startViewEvent, flashcatContext, writeScope, writer)
            }
        }
    ...
    private fun sendApplicationStartEvent(
        ...
        if (isForegroundProcess) {
            ...
            delegateToChildren(appStartedEvent, flashcatContext, writeScope, writer)
