...
import com.flashcat.rum.rum.assertj.ActionEventAssert.Companion.assertThat
import com.flashcat.rum.rum.assertj.ErrorEventAssert.Companion.assertThat
import com.flashcat.rum.rum.assertj.LongTaskEventAssert.Companion.assertThat
import com.flashcat.rum.rum.assertj.ViewEventAssert.Companion.assertThat
import com.flashcat.rum.rum.assertj.VitalEventAssert
import com.flashcat.rum.rum.assertj.VitalFeatureOperationPropertiesAssert
...
import com.flashcat.rum.rum.internal.anr.ANRException
...
import com.flashcat.rum.rum.internal.metric.ViewEndedMetricDispatcher
...
import com.flashcat.rum.rum.internal.metric.ViewMetricDispatcher
import com.flashcat.rum.rum.internal.metric.interactiontonextview.InteractionToNextViewMetricResolver
import com.flashcat.rum.rum.internal.metric.interactiontonextview.InternalInteractionContext
import com.flashcat.rum.rum.internal.metric.networksettled.InternalResourceContext
import com.flashcat.rum.rum.internal.metric.networksettled.NetworkSettledMetricResolver
...
import com.flashcat.rum.rum.internal.toError
import com.flashcat.rum.rum.internal.toLongTask
import com.flashcat.rum.rum.internal.toView
import com.flashcat.rum.rum.internal.toVital
import com.flashcat.rum.rum.internal.utils.buildDDTagsString
import com.flashcat.rum.rum.internal.vitals.VitalInfo
import com.flashcat.rum.rum.internal.vitals.VitalListener
...
internal class RumViewScopeTest {
    ...
    fun `M update the context with viewType NONE W handleEvent(StopView)`(
        ...
        testedScope.handleEvent(fakeEvent, fakeFlashcatContext, mockEventWriteScope, mockWriter)
    ...
    fun `M keep hasReplay=true flag if set W getRumContext() { events processed }`(
        ...
        val flashcatContextWithReplay = fakeFlashcatContext.copy(
        ...
        whenever(
            mockFeaturesContextResolver.resolveViewHasReplay(
                flashcatContextWithReplay,
                testedScope.viewId
            )
        ) doReturn fakeHasReplay
        testedScope.handleEvent(fakeEvent, flashcatContextWithReplay, mockEventWriteScope, mockWriter)
        val fakeStopEvent = RumRawEvent.StopView(fakeKey, forge.exhaustiveAttributes())
        val stopViewContext = fakeFlashcatContext.copy(
            featuresContext = fakeFlashcatContext.featuresContext +
    ...
    fun `M send event W handleEvent(StartView) on active view`(
        ...
        argumentCaptor<ViewEvent> {
            ...
            assertThat(lastValue).apply {
                ...
                hasDeviceInfo(
                    fakeFlashcatContext.deviceInfo.deviceName,
                    fakeFlashcatContext.deviceInfo.deviceModel,
                    fakeFlashcatContext.deviceInfo.deviceBrand,
                    fakeFlashcatContext.deviceInfo.deviceType.toViewSchemaType(),
                    fakeFlashcatContext.deviceInfo.architecture
                )
                hasOsInfo(
                    fakeFlashcatContext.deviceInfo.osName,
                    fakeFlashcatContext.deviceInfo.osVersion,
                    fakeFlashcatContext.deviceInfo.osMajorVersion
                )
                hasSlownessInfo(
                    fakeSlowRecords,
                    fakeSlownessRate,
                ...
                hasConnectivityInfo(fakeFlashcatContext.networkInfo)
                hasServiceName(fakeFlashcatContext.service)
                hasVersion(fakeFlashcatContext.version)
    ...
    fun `M send event W handleEvent(ErrorSent) on active view`(
        ...
        // When
        val result = testedScope.handleEvent(fakeEvent, fakeFlashcatContext, mockEventWriteScope, mockWriter)
    ...
    fun `M send event W handleEvent(ResourceSent) on active view`(
        ...
        // When
        val result = testedScope.handleEvent(fakeResourceEvent, fakeFlashcatContext, mockEventWriteScope, mockWriter)
            ...
            assertThat(firstValue)
                .apply {
                    ...
                    hasDeviceInfo(
                        ...
                        fakeFlashcatContext.deviceInfo.deviceModel,
                        fakeFlashcatContext.deviceInfo.deviceBrand,
                        fakeFlashcatContext.deviceInfo.deviceType.toActionSchemaType(),
                        fakeFlashcatContext.deviceInfo.architecture
                    )
                    hasOsInfo(
                        fakeFlashcatContext.deviceInfo.osName,
    ...
    fun `M send event W handleEvent(ResourceSent) on stopped view`(
        ...
        // When
        val result = testedScope.handleEvent(fakeResourceSent, fakeFlashcatContext, mockEventWriteScope, mockWriter)
    ...
    fun `M close the scope W handleEvent(ActionSent) on stopped view { ApplicationStarted }`(
        ...
        // When
        testedScope.handleEvent(fakeEvent, fakeFlashcatContext, mockEventWriteScope, mockWriter)
        val result = testedScope.handleEvent(fakeActionSent, fakeFlashcatContext, mockEventWriteScope, mockWriter)
    ...
    fun `M do nothing + log warning W handleEvent(StartAction+!CUSTOM)+active child ActionScope`(
        ...
        // Then
        verify(mockChildScope).handleEvent(fakeEvent, fakeFlashcatContext, mockEventWriteScope, mockWriter)
        verifyNoInteractions(mockWriter)
        assertThat(result).isSameAs(testedScope)
    ...
    fun `M create ResourceScope with active actionId W handleEvent(StartResource)`(
        ...
        // Then
        verify(mockActionScope).handleEvent(fakeEvent, fakeFlashcatContext, mockEventWriteScope, mockWriter)
        verifyNoInteractions(mockWriter)
        assertThat(result).isSameAs(testedScope)
        assertThat(testedScope.activeResourceScopes).isNotEmpty()
        val entry = testedScope.activeResourceScopes.entries.first()
    ...
    fun `M decrease pending Resource W handleEvent(ResourceDropped) on active view`(
        ...
        // When
        val result = testedScope.handleEvent(fakeResourceDrooped, fakeFlashcatContext, mockEventWriteScope, mockWriter)
    ...
    fun `M decrease pending Resource W handleEvent(ResourceDropped) on stopped view`(
        ...
        // When
        val result = testedScope.handleEvent(fakeResourceDropped, fakeFlashcatContext, mockEventWriteScope, mockWriter)
            ...
            assertThat(firstValue)
                .apply {
                    ...
                    hasDeviceInfo(
                        ...
                        fakeFlashcatContext.deviceInfo.deviceModel,
                        fakeFlashcatContext.deviceInfo.deviceBrand,
                        fakeFlashcatContext.deviceInfo.deviceType.toErrorSchemaType(),
                        fakeFlashcatContext.deviceInfo.architecture
                    )
                    hasOsInfo(
                        fakeFlashcatContext.deviceInfo.osName,
    ...
    fun `M send event W handleEvent(AddError) {internal is_crash=true}`(
        ...
        val result = testedScope
            .handleEvent(fakeEvent, fakeFlashcatContext, mockEventWriteScope, mockWriter)
            ?.handleEvent(fakeNativeCrashEvent, fakeFlashcatContext, mockEventWriteScope, mockWriter)
            ...
            assertThat(firstValue)
                .apply {
                    ...
                    hasDeviceInfo(
                        ...
                        fakeFlashcatContext.deviceInfo.deviceModel,
                        fakeFlashcatContext.deviceInfo.deviceBrand,
                        fakeFlashcatContext.deviceInfo.deviceType.toLongTaskSchemaType(),
                        fakeFlashcatContext.deviceInfo.architecture
                    )
                    hasOsInfo(
                        fakeFlashcatContext.deviceInfo.osName,
...
// endregion
    ...
    fun `M unregister vital monitors W handleEvent(StopSession)`() {
        ...
        // When
        testedScope.handleEvent(RumRawEvent.StopSession(), fakeFlashcatContext, mockEventWriteScope, mockWriter)
    ...
    fun `M update the duration to 1ns W handleEvent { computed duration equal to 0 }`(
        ...
        // When
        testedScope.handleEvent(rawEventData.event, fakeFlashcatContext, mockEventWriteScope, mockWriter)
    ...
    fun `M not update the duration W handleEvent { after view stop }`(
        ...
        // When
        testedScope.handleEvent(stopEvent, fakeFlashcatContext, mockEventWriteScope, mockWriter)
        testedScope.handleEvent(otherEvent, fakeFlashcatContext, mockEventWriteScope, mockWriter)
    ...
    fun `M call resolveReport(viewId, false) of slowFramesListener W handleEvent()`(forge: Forge) {
        ...
        // When
        testedScope.handleEvent(event, fakeFlashcatContext, mockEventWriteScope, mockWriter)
    ...
    fun `M produce event safe for serialization W handleEvent()`(
        ...
        testedScope.handleEvent(forge.stopViewEvent(), fakeFlashcatContext, mockEventWriteScope, mockWriter)
    ...
    fun `M sendViewEnded W handleEvent(StartView)`(forge: Forge) {
        ...
        // When
        testedScope.handleEvent(forge.startViewEvent(), fakeFlashcatContext, mockEventWriteScope, mockWriter)
        ...
        argumentCaptor<RumVitalOperationStepEvent> {
            ...
            VitalEventAssert.assertThat(lastValue)
                ...
                .hasDeviceInfo(
                    ...
                    fakeFlashcatContext.deviceInfo.deviceModel,
                    fakeFlashcatContext.deviceInfo.deviceBrand,
                    fakeFlashcatContext.deviceInfo.deviceType.toVitalSchemaType(),
                    fakeFlashcatContext.deviceInfo.architecture
                )
                .hasOsInfo(
                    fakeFlashcatContext.deviceInfo.osName,
                ...
                .hasVersion(fakeFlashcatContext.version)
                .hasServiceName(fakeFlashcatContext.service)
                .hasDDTags(buildDDTagsString(fakeFlashcatContext))
