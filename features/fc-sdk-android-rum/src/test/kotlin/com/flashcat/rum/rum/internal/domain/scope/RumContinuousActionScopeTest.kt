...
internal class RumContinuousActionScopeTest {
    ...
    fun `M send Action with updated data W handleEvent(StopAction+any) {viewTreeChangeCount!=0}`(
        ...
        Thread.sleep(TEST_INACTIVITY_MS * 2)
        val result2 = testedScope.handleEvent(mockEvent(), fakeFlashcatContext, mockEventWriteScope, mockWriter)
    ...
    fun `M send Action after threshold W handleEvent(StartResource+StopAction+StopResource+any)`(
        ...
        val result2 = testedScope.handleEvent(fakeEvent, fakeFlashcatContext, mockEventWriteScope, mockWriter)
        Thread.sleep(TEST_INACTIVITY_MS * 2)
        fakeEvent = RumRawEvent.StopResource(key, statusCode, size, kind, emptyMap())
        val result3 = testedScope.handleEvent(fakeEvent, fakeFlashcatContext, mockEventWriteScope, mockWriter)
        Thread.sleep(TEST_INACTIVITY_MS * 2)
        val result4 = testedScope.handleEvent(mockEvent(), fakeFlashcatContext, mockEventWriteScope, mockWriter)
    ...
    fun `M send Action W handleEvent(StartResource+StopAction+any) missing resource key`(
        ...
        key = null
        System.gc()
        val result3 = testedScope.handleEvent(mockEvent(), fakeFlashcatContext, mockEventWriteScope, mockWriter)
