...
internal class RumApplicationScopeTest {
    ...
    fun `delegates all events to child scope`() {
        ...
        verify(mockChildScope).handleEvent(mockEvent, fakeFlashcatContext, mockEventWriteScope, mockWriter)
        verifyNoInteractions(mockWriter)
    }
    ...
    fun `M send ApplicationStarted event once W handleEvent { app is in foreground }`(
        ...
        // When
        fakeEvents.forEach {
            testedScope.handleEvent(it, fakeFlashcatContext, mockEventWriteScope, mockWriter)
        }
    ...
    fun `M not send ApplicationStarted event W handleEvent { SdkInit event }`(
        ...
        // When
        testedScope.handleEvent(fakeSdkInitEvent, fakeFlashcatContext, mockEventWriteScope, mockWriter)
