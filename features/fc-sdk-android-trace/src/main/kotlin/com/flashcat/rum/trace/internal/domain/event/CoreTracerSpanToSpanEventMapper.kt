...
internal class CoreTracerSpanToSpanEventMapper(
    ...
    override fun map(flashcatContext: FlashcatContext, model: DDSpan): SpanEvent {
        val serverOffset = flashcatContext.time.serverTimeOffsetNs
        val metrics = resolveMetrics(model)
        val metadata = resolveMeta(flashcatContext, model)
    ...
    // TODO RUM-10805 - make it back private and re-create objects in tests
    internal fun resolveMeta(flashcatContext: FlashcatContext, event: DDSpan): SpanEvent.Meta {
        val deviceInfo = resolveDeviceInfo(flashcatContext.deviceInfo)
        val osInfo = resolveOsInfo(flashcatContext.deviceInfo)
        val networkInfoMeta = if (networkInfoEnabled) resolveNetworkInfo(flashcatContext.networkInfo) else null
        val userInfo = flashcatContext.userInfo
        val accountInfo = flashcatContext.accountInfo
        ...
        meta[APPLICATION_VARIANT_KEY] = flashcatContext.variant
