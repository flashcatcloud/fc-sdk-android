...
internal class SpanEventSerializerTest {
    ...
    fun `M serialize a SpanEvent W serialize`(@Forgery fakeSpanEvent: SpanEvent) {
        // WHEN
        val serialized = testedSerializer.serialize(fakeFlashcatContext, fakeSpanEvent)
        ...
        Assertions.assertThat(jsonObject.get(KEY_ENV).asString).isEqualTo(fakeFlashcatContext.env)
    }
    ...
    fun `M not throw W serialize() { usr#additionalProperties serialization throws }`(
        ...
        // WHEN
        val serialized = testedSerializer.serialize(fakeFlashcatContext, faultySpanEvent)
