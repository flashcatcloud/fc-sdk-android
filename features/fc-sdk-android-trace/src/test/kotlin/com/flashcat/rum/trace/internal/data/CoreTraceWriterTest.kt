...
internal class CoreTraceWriterTest {
    ...
    fun `set up`(forge: Forge) {
        ...
        whenever(mockLegacyMapper.map(eq(fakeFlashcatContext), any())) doReturn forge.getForgery<SpanEvent>()
        whenever(mockSerializer.serialize(eq(fakeFlashcatContext), any())) doReturn forge.aString()
    ...
    fun `M write spans W write()`(forge: Forge) {
        ...
        ddSpans.forEachIndexed { index, ddSpan ->
            whenever(mockLegacyMapper.map(fakeFlashcatContext, ddSpan)) doReturn spanEvents[index]
        }

        spanEvents.forEachIndexed { index, spanEvent ->
            whenever(
                mockSerializer.serialize(fakeFlashcatContext, spanEvent)
    ...
    fun `M write spans W write() { with RUM context }`(
        ...
        val fakeInitialFlashcatContext = forge.aFlashcatContextWithRumContext(fakeRumContext)
        val fakeLazyContext = completedFutureMock(fakeInitialFlashcatContext)
        ...
        argumentCaptor<DDSpan> {
            verify(mockLegacyMapper, times(ddSpans.size)).map(eq(fakeFlashcatContext), capture())
    ...
    fun `M write spans W write() { without RUM context when empty }`(
        ...
        // GIVEN
        val fakeInitialFlashcatContext = forge.aFlashcatContextWithRumContext(emptyMap())
        val fakeLazyContext = mock<Future<FlashcatContext>> { on { get() } doReturn fakeInitialFlashcatContext }
        val ddSpans = createNonEmptyDdSpans(forge, includeDropSamplingPriority = false).map { span ->
            span.withLazyContext(fakeLazyContext)
        }
    ...
    fun `M write spans W write() { without RUM context, lazy Datadog context is not complete }`(
        ...
        // GIVEN
        val fakeLazyContext = mock<Future<FlashcatContext>> { on { get() } doReturn null }
        val ddSpans = createNonEmptyDdSpans(forge, includeDropSamplingPriority = false).map { span ->
            span.withLazyContext(fakeLazyContext)
        }
