...
import cloud.flashcat.android.rum.internal.RumFeature.Configuration
...
import cloud.flashcat.android.rum.internal.utils.HUNDRED
import cloud.flashcat.android.rum.internal.utils.percent
...
import cloud.flashcat.android.telemetry.model.TelemetryUsageEvent.ActionType
...
internal class TelemetryEventHandler(
    ...
    private fun isGlobalTracerRegistered(): Boolean {
        // We don't reference cloud.flashcat.android.trace from RUM directly, so using reflection for this.
        // Would be nice to add the test with the flavor which is has no cloud.flashcat.android.trace and test
        // for obfuscation enabled case.
        return try {
            val globalDatadogTracer =
                Class.forName("cloud.flashcat.android.trace.GlobalDatadogTracer")
            return try {
