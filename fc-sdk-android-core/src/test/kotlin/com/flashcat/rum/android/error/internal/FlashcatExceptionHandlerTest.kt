...
import com.flashcat.rum.core.UploadWorker
import com.flashcat.rum.core.feature.event.JvmCrash
import com.flashcat.rum.core.internal.CoreFeature
import com.flashcat.rum.core.internal.thread.waitToIdle
import com.flashcat.rum.core.internal.utils.TAG_DATADOG_UPLOAD
import com.flashcat.rum.core.internal.utils.UPLOAD_WORKER_NAME
...
internal class DatadogExceptionHandlerTest {
    ...
    private fun setDefaultWorkManager(instance: WorkManagerImpl?) {
        try {
            WorkManagerImpl::class.java.setStaticValue("sDefaultInstance", instance)
        } catch (_: UnsupportedOperationException) {
            // Newer WorkManager versions might not expose this field; skip test in that case.
            assumeTrue(false, "WorkManagerImpl.sDefaultInstance is not available on this WorkManager version")
        }
    }
    ...
    fun `tear down`() {
        ...
        runCatching { WorkManagerImpl::class.java.setStaticValue("sDefaultInstance", null) }
    ...
    fun `M schedule the worker W logging an exception`() {
        ...
        setDefaultWorkManager(mockWorkManager)
        Thread.setDefaultUncaughtExceptionHandler(null)
        testedHandler.register()
        val currentThread = Thread.currentThread()
