...
import com.flashcat.rum.core.internal.NoOpContextProvider
...
/**
 * This region groups all test about FlashcatCore instance (except Initialization).
 */
@Extensions(
...
internal class FlashcatCoreTest {

    private lateinit var testedCore: FlashcatCore
    ...
    fun `M notify no feature registered W setEventReceiver() { feature is not registered }`(
        ...
        mockInternalLogger.verifyLog(
            ...
            FlashcatCore.MISSING_FEATURE_FOR_EVENT_RECEIVER.format(Locale.US, feature)
        )
    }
    ...
    fun `M notify receiver exists W setEventReceiver() { feature already has receiver }`(
        ...
        mockInternalLogger.verifyLog(
            InternalLogger.Level.WARN,
            InternalLogger.Target.USER,
            FlashcatCore.EVENT_RECEIVER_ALREADY_EXISTS.format(Locale.US, feature)
        )
    }
    ...
    fun `M notify listener already registered W setContextUpdateReceiver()`() {
        ...
        mockInternalLogger.verifyLog(
            InternalLogger.Level.WARN,
            InternalLogger.Target.USER,
            FlashcatCore.CONTEXT_UPDATE_LISTENER_ALREADY_REGISTERED.format(Locale.US, mockContextUpdateListener)
        )
    }
    ...
    fun `M provide time info without correction W time() {NoOpTimeProvider}`() {
        ...
        // the FlashcatCore.time implementation computes server and device time independently and it can sometimes
    ...
    fun `M log info when writing last view event W writeLastViewEvent(){ below R and no NDK feature }`(
        ...
        mockInternalLogger.verifyLog(
            ...
            FlashcatCore.NO_NEED_TO_WRITE_LAST_VIEW_EVENT
        )
    }
