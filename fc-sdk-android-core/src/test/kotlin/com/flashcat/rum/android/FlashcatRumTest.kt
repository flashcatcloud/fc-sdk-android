...
@ProhibitLeavingStaticMocksIn(FlashcatRum::class)
internal class FlashcatRumTest {

    @Mock
    lateinit var mockConnectivityMgr: ConnectivityManager

    @Forgery
    lateinit var fakeConfiguration: FlashcatConfig
    ...
    fun `M return sdk instance W initialize() + getInstance()`() {
        // When
        val initialized = FlashcatRum.initialize(
            appContext.mockInstance,
            fakeConfiguration,
            fakeConsent
        )
        val instance = FlashcatRum.getInstance()
    ...
    fun `M return sdk instance W initialize(name) + getInstance(name)`(
        ...
        val instance = FlashcatRum.getInstance(name)
    ...
    fun `M warn W initialize() + initialize()`() {
        // When
        val initialized1 = FlashcatRum.initialize(
            appContext.mockInstance,
            fakeConfiguration,
            fakeConsent
        )
        val initialized2 = FlashcatRum.initialize(
            appContext.mockInstance,
            fakeConfiguration,
            fakeConsent
        )
        ...
        logger.mockInternalLogger.verifyLog(
            ...
            FlashcatRum.MESSAGE_ALREADY_INITIALIZED
        )
    ...
    fun `M warn W initialize(name) + initialize(name)`(
        ...
        // When
        FlashcatRum.initialize(name, appContext.mockInstance, fakeConfiguration, fakeConsent)
        FlashcatRum.initialize(name, appContext.mockInstance, fakeConfiguration, fakeConsent)
    ...
    fun `M create instance ID W initialize()`(
        @Forgery fakeConfiguration: FlashcatConfig,
        ...
        // When
        val instance = FlashcatRum.initialize(
            appContext.mockInstance,
            fakeConfiguration,
            fakeConsent
        )
    ...
    fun `M warn W initialize() {hash generator fails}`() {
        ...
        logger.mockInternalLogger.verifyLog(
            ...
            InternalLogger.Target.USER,
            FlashcatRum.CANNOT_CREATE_SDK_INSTANCE_ID_ERROR
        )
    ...
    fun `M stop specific instance W stopInstance()`() {
        // Given
        val sdk = FlashcatRum.initialize(
            appContext.mockInstance,
            fakeConfiguration,
            fakeConsent
        ) as? DatadogCore
        ...
        // When
        FlashcatRum.stopInstance()
        val getInstance = FlashcatRum.getInstance()
    ...
    fun `M stop specific instance W stopInstance(name)`(
        ...
        // When
        FlashcatRum.stopInstance(name)
        val getInstance = FlashcatRum.getInstance(name)
    ...
    fun `M not stop specific instance W stopInstance(name) {different name}`(
        ...
        // When
        FlashcatRum.stopInstance(name2)
        val getInstance = FlashcatRum.getInstance(name)
    ...
    fun `M warn W getInstance() { instance is not initialized }`(
        ...
        // When
        FlashcatRum.getInstance(fakeInstanceName)
        ...
        argumentCaptor<() -> String> {
            ...
            assertThat(filteredActualMessage)
                .isEqualTo(
                    FlashcatRum.MESSAGE_SDK_NOT_INITIALIZED.format(
                        Locale.US,
    ...
    fun `M return false W isInitialized() { instance is not initialized }`(
        ...
        // When
        val result = FlashcatRum.isInitialized(fakeInstanceName)
    ...
    fun `M set and get lib verbosity W setVerbosity() + getVerbosity()`(
        ...
        // When
        FlashcatRum.setVerbosity(level)
        val result = FlashcatRum.getVerbosity()
    ...
    fun `M set tracking consent W setTrackingConsent()`(
        ...
        // When
        FlashcatRum.setTrackingConsent(fakeTrackingConsent, mockSdkCore)
    ...
    fun `M set user info W setUserInfo()`(
        ...
        // When
        FlashcatRum.setUserInfo(id, name, email, fakeUserProperties, mockSdkCore)
    ...
    fun `M add user properties W addUserProperties()`(
        ...
        // When
        FlashcatRum.addUserProperties(fakeUserProperties, mockSdkCore)
    ...
    fun `M clear user info W clearUserInfo()`() {
        ...
        // When
        FlashcatRum.clearUserInfo(mockSdkCore)
    ...
    fun `M clear all data W clearAllData()`() {
        ...
        // When
        FlashcatRum.clearAllData(mockSdkCore)
    ...
    fun `M call Core add account extra info W addAccountExtraInfo()`(
        ...
        // When
        FlashcatRum.addAccountExtraInfo(
            extraInfo = fakeExtraInfo,
            sdkCore = mockSdkCore
        )
    ...
    fun `M call Core clear account info W clearAccountInfo()`() {
        ...
        // When
        FlashcatRum.clearAccountInfo(sdkCore = mockSdkCore)
