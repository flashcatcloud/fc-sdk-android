...
import com.flashcat.rum.core.internal.SdkFeature.Companion.BATCH_COUNT_METRIC_NAME
import com.flashcat.rum.core.internal.SdkFeature.Companion.METER_NAME
...
import com.flashcat.rum.core.internal.data.upload.DataUploadRunnable
...
import com.flashcat.rum.core.internal.metrics.BatchMetricsDispatcher.Companion.TRACK_KEY
...
import com.flashcat.rum.core.internal.persistence.file.batch.BatchFileOrchestrator
...
import com.flashcat.rum.utils.config.CoreFeatureTestConfiguration
...
internal class SdkFeatureTest {
    ...
    fun `M provide write context W withWriteContext(callback)`(
        @Forgery fakeContext: FlashcatContext,
        @StringForgery fakeWithFeatureContexts: Set<String>,
        @Mock mockEventWriteScope: EventWriteScope
    ) {
        // Given
        testedFeature.storage = mockStorage
        val callback = mock<(FlashcatContext, EventWriteScope) -> Unit>()
    ...
    fun `M provide Datadog context W withContext(callback)`(
        ...
        // Given
        testedFeature.storage = mockStorage
        val callback = mock<(FlashcatContext) -> Unit>()
        whenever(mockContextProvider.getContext(fakeWithFeatureContexts)) doReturn fakeContext
    ...
    fun `M provide write context W getWriteContextSync()`(
        ...
        whenever(coreFeature.mockInstance.contextExecutorService.submit(any<Callable<*>>())) doAnswer {
            val callable = it.getArgument<Callable<Pair<FlashcatContext, EventWriteScope>>>(0)
            mock<Future<*>>().apply {
