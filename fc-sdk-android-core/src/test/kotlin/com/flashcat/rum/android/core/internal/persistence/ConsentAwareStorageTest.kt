...
import com.flashcat.rum.core.internal.data.upload.DataOkHttpUploader.Companion.HTTP_ACCEPTED
import com.flashcat.rum.core.internal.metrics.BenchmarkUploads
...
internal class ConsentAwareStorageTest {
    ...
    fun `M provide writer W getEventWriteScope() {consent=granted}`() {
        ...
        fakeflashcatContext = fakeflashcatContext.copy(trackingConsent = TrackingConsent.GRANTED)

        // When
        testedStorage.getEventWriteScope(fakeflashcatContext)
            .invoke(mockCallback)
    ...
    fun `M provide writer W getEventWriteScope() {consent=pending}`() {
        // Given
        val mockCallback = mock<(EventBatchWriter) -> Unit>()
        fakeflashcatContext = fakeflashcatContext.copy(trackingConsent = TrackingConsent.PENDING)
    ...
    fun `M provide no-op writer W getEventWriteScope() {not_granted}`() {
        // Given
        val mockCallback = mock<(EventBatchWriter) -> Unit>()
        fakeflashcatContext = fakeflashcatContext.copy(trackingConsent = TrackingConsent.NOT_GRANTED)
    ...
    fun `M log error W getEventWriteScope() { task was rejected }`() {
        ...
        // When
        testedStorage.getEventWriteScope(fakeflashcatContext).invoke {
            // no-op
        }
