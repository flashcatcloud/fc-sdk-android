...
import com.flashcat.rum.okhttp.TraceContextInjection
import com.flashcat.rum.okhttp.internal.trace.toInternalTracingHeaderType
import com.flashcat.rum.okhttp.internal.utils.forge.OkHttpConfigurator
import com.flashcat.rum.okhttp.trace.TracingInterceptor.Companion.OKHTTP_INTERCEPTOR_HEADER_TYPES
import com.flashcat.rum.okhttp.trace.TracingInterceptor.Companion.OKHTTP_INTERCEPTOR_SAMPLE_RATE
import com.flashcat.rum.okhttp.utils.assertj.HeadersAssert.Companion.assertThat
import com.flashcat.rum.okhttp.utils.config.DatadogSingletonTestConfiguration
import com.flashcat.rum.okhttp.utils.config.GlobalRumMonitorTestConfiguration
import com.flashcat.rum.okhttp.utils.verifyLog
import com.flashcat.rum.trace.TracingHeaderType
import com.flashcat.rum.trace.api.DatadogTracingConstants.PrioritySampling
...
import com.flashcat.rum.trace.internal.fromHex
...
internal open class TracingInterceptorTest {
    ...
    fun `M inject datadog headers W intercept() {global known host + not sampled}`(
        ...
        // Given
        val nonflashcatContextKey = forge.anAlphabeticalString()
        val nonflashcatContextKeyValue = forge.anAlphabeticalString()
        val flashcatContext = listOf(
        ...
        mockPropagation.wheneverInjectCalledPassContextToHeaders(
            flashcatContext,
            nonflashcatContextKey,
            nonflashcatContextKeyValue
        )
        whenever(mockTraceSampler.sample(mockSpan)).thenReturn(false)
        whenever(mockResolver.isFirstPartyUrl(fakeUrl.toHttpUrl())).thenReturn(true)
        whenever(mockResolver.headerTypesForUrl(fakeUrl.toHttpUrl())).thenReturn(
        ...
        argumentCaptor<Request> {
            ...
            flashcatContext.forEach {
                assertThat(lastValue.header(it.key)).isEqualTo(it.value)
            }
            assertThat(lastValue.header(nonflashcatContextKey)).isNull()
    ...
    @Test
    fun `M blocks on flashcatContext future W intercept { new values intersects existing values }`(
        @IntForgery(min = 200, max = 600) statusCode: Int,
        forge: Forge
    ) {
        // Given
        val mockFuture = mock<FutureFlashcatContext>>()
        fakeRequest = forgeRequest(forge)
